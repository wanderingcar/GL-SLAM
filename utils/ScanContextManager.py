...
def pt2rs(point, gap_ring, gap_sector, num_ring, num_sector):
    ...
    idx_ring = np.divmod(faraway, gap_ring)[0]  # 몫
    idx_sector = np.divmod(theta, gap_sector)[0]  # 몫
...
def ptcloud2sc(ptcloud, sc_shape, max_length):  # ptcloud : mX2 np array,
    ...
    for pt_idx in range(num_points):
        ...
        if sc_counter[idx_ring, idx_sector] >= enough_large:
            continue
        sc_storage[int(sc_counter[idx_ring, idx_sector]), idx_ring, idx_sector] = 1
...
class ScanContextManager:
    def __init__(self, shape=None, num_candidates=10,
                 threshold=0.15):  # default configs are same as the original paper
        if shape is None:
            shape = [20, 60]
        self.shape = shape
    ...
    def detectLoop(self):
        ...
        if valid_recent_node_idx < 1:
            ...
            for ith in range(self.num_candidates):
                ...
                dist, theta = distance_sc(candidate_sc, query_sc)
                if dist < nn_dist:
                    nn_dist = dist
                    nn_theta = theta
            ...
            if nn_dist < self.threshold:
                nn_theta_deg = nn_theta * (360 / self.shape[1])
                return nn_idx, nn_dist, nn_theta_deg  # loop detected!
            else:
                return None, None, None
