...
class PoseGraphManager:
    def __init__(self):
        self.prior_cov = minisam.DiagonalLoss.Sigmas(np.array([1e-6, 1e-6, 1e-4, 1e-4]))
        self.const_cov = np.array([0.5, 0.5, 0.1, 0.1])
    ...
    def addPriorFactor(self):
        ...
        self.graph_initials.add(minisam.key('x', self.curr_node_idx), minisam.SE2(self.curr_se2))
        self.graph_factors.add(minisam.PriorFactor(
            minisam.key('x', self.curr_node_idx),
            minisam.SE2(self.curr_se2),
            self.prior_cov))
    ...
    def addLoopFactor(self, loop_transform, loop_idx):
        self.graph_factors.add(minisam.BetweenFactor(
            ...
            minisam.key('x', self.curr_node_idx),
            minisam.SE2(loop_transform), self.loop_cov))
    ...
    def optimizePoseGraph(self):
        ...
        self.curr_se2[:2, :2] = pose_rot
        self.curr_se2[:2, 2] = pose_trans
